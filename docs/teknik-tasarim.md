...
### Koruma katmanları (hepsi kodda uygulanmıştır)

1. Backend bütün paneli **deterministic metne** dönüştürür (`AnomalySummaryBuilder`).
2. Prompt, modele durumları **kesin kabul et, yeniden hesaplama, veri uydurma** der.
3. Ollama `temperature = 0`, `stream = false`, `format = json` ile çağrılır.
4. Model JSON'u **parse edilir**; boş özet / bozuk çıktı **reddedilir** ve cache'e yazılmaz.
5. Modelin döndürdüğü `flaggedTests` **kullanılmaz**; backend'in listesi kullanılır.
6. Disclaimer modelden beklenmez, **backend ekler**.
7. Timeout / bağlantı hatası → kontrollü **`503 AI analysis unavailable`**; sistemin kalanı çalışır.
8. Sonuç `(sample, model, promptVersion)` ile **cache**'lenir; ikinci çağrıda LLM çağrılmaz.
...
mode'ları** kapsamasıdır. Güncel: **backend 43 · mock 9 · frontend 14**, tümü yeşil.
...
| Prompt injection (kimlik alanında newline) | Sahte prompt satırı enjekte edilemez | `AnomalySummaryBuilderTest` |
