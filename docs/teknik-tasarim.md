...
#### İlk yaklaşım neden hatalıydı?

İlk geliştirme sürümünde demo kurulumunu tek komutla çalıştırabilmek için `application-dev.yml` içinde
sabit bir dev anahtarı ve Docker konfigürasyonunda tahmin edilebilir bir fallback anahtarı vardı.
Bu yaklaşım "yalnızca demo secret'ı" olarak düşünülmüştü; fakat JWT imzalama anahtarının etkisi demo
parolasından farklıdır. Bilinen demo parola yalnız o hesabı açarken, bilinen HMAC anahtarı saldırgana
istediği kullanıcı ve rol claim'leriyle geçerli token üretme yetkisi verir.

Örneğin saldırgan public repodaki anahtarı alıp `sub=doctor`, `role=DOCTOR` ve gelecekte bir expiry
içeren token imzalayabilirdi. Backend imzayı geçerli göreceği için `/api/auth/login` endpoint'ine ve
BCrypt parola kontrolüne hiç uğramadan korumalı endpoint'lere erişebilirdi. Bu nedenle hardcoded veya
public fallback JWT secret, login mekanizmasını varmış gibi gösteren fakat fiilen bypass edilebilir
hale getiren kritik bir tasarım hatasıdır.

Hata fark edildikten sonra public/default secret kaldırıldı. Yapılan değişiklik yalnız konfigürasyon
temizliği değil, güvenli varsayılana geçiştir: geliştirici secret vermeyi unutursa sistem güvensiz bir
anahtarla çalışmak yerine fail-fast davranarak hiç başlamaz.
...
Bir secret git geçmişine girdikten sonra mevcut dosyadan kaldırılması onu gizli hale getirmez; eski
commit'lerden hâlâ okunabilir. Gerçek bir incident response sürecinde ilgili anahtar compromised kabul
edilir, çalışan bütün ortamlarda rotate edilir, mevcut token'lar geçersiz kılınır ve gerekiyorsa
`git filter-repo` gibi bir araçla public geçmiş temizlenir. Bu projede runtime secret zorunluluğuna
geçiş yeni kurulumların public anahtarla başlamasını engeller; production tesliminde rotation ve
history temizliği ayrıca tamamlanmalıdır.
